    ...
    def on_update_data_table(
        ...
        if triggered:
            ...
            for energy_source in ET_IDX:
                ...
                df = pd.DataFrame(
                    index=YEARS_IDX, columns=BL_IDX)
                ...
                columns = [{"name": i, "id": i, "type": "numeric",
                            'format': Format(group=',')} for i in df.columns]
                columns.pop(0)
                columns.insert(0, {"name": "Jahre", "id": "Jahre"})

                databars = [data_bars(df.iloc[:-1, :], BL, provinces_color_table[BL]+"4D")
                            for BL in BL_IDX]
                dbars = []
                for db in databars:
                    dbars += db
