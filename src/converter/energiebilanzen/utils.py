...
from converter.energiebilanzen.check_errors import (
    check_column_errors,
...
def preprocess_res_sheet(res_sheet: pd.DataFrame, res_conversion_factors: pd.Series):

    # Exclude conversion unit and multiplicator columns
    res_sheet = res_sheet.iloc[:, :-2]
    ...
    res_sheet.columns = res_sheet.iloc[2, :].values
    res_sheet.reset_index(drop=True, inplace=True)
    res_sheet = res_sheet.iloc[3:70, :]
    res_sheet.reset_index(drop=True, inplace=True)

    # Get rid of in index column
    res_sheet = res_sheet.iloc[:, 1:]
    ...
    # res_sheet_index = res_sheet.iloc[:, 1]
...
def create_indices(provinces: List, last_year: int):
    ...
    eb_row_midx = None
    res_row_midx = None
    for (midx, path, sheet_name) in zip(
        [eb_row_midx, res_row_midx],
        [file_paths["midx_eb"], file_paths["midx_res"]],
        ["MIDX_EB", "MIDX_RES"],
    ):
        ...
        # # _____________________________________________________________ EB ROW MIDX
        # eb_row_midx = pd.MultiIndex.from_tuples(
        #     tuples=[
        #         tuple(x)
        #         for x in pd.read_excel(
        #             io=file_paths["midx_eb"],
        #             sheet_name="MIDX_EB",
        #             na_filter=False,
        #             header=None,
        #         ).values
        ...
        # # ____________________________________________________________ RES ROW MIDX
    ...
    res_conversion_factors = res_row_midx.get_level_values("IDX_4")

    print("conversion_multiplicator: ", res_conversion_factors)
...
def copy_eb_data(
    ...
    for enum, energy_source in enumerate(eb_sheet_names):
        ...
        eb_df.at[IDX[:], IDX[province, energy_source, :]] = eb_data  #
...
def copy_res_data(
    ...
    res_data = (
        res_sheet.set_index(res_sheet.columns[0], drop=True)
        .apply(pd.to_numeric, errors="coerce")
        .round(2)
    )
    ...
    # res_data.sort_index(inplace=True, axis="columns").lex_sorted

    res_data.index, res_data.columns = res_row_midx, res_midx_col

    # eb_data.sort_index(inplace=True, axis="rows", level=0)

    # Copy res data to container res_df
    res_df.at[IDX[:], IDX[province, :]] = res_data  #
