...
def test_stacked_energy_sources_per_year(
    test_dataset,
    test_nea_workbook,
    test_provinces,
    ...
    data_objects = [_data for _data in ds.objects.filter(per_energy_source=True,)]

    test_write_to_xlsx(
        wb=test_nea_workbook, data_objects=data_objects, sheet_name="STACKED_ES",
    )


@pytest.mark.dependency(depends=["test_stacked_energy_sources_per_year"])
