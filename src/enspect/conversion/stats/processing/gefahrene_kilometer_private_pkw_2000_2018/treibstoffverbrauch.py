...
file = Path(
    "C:/Code/enerbal/src/files/stats/processing/gefahrene_kilometer_private_pkw_2000_2018/fahrleistungen_privater_pkw.xlsx"
)
sheet_names = [
    "Österreich",
    "Burgenland",
...
for enum, sheet_name in enumerate(sheet_names):
    ...
    df_fuel_liter = pd.DataFrame(columns=index, index=["Benzin", "Diesel", "Zusammen"])

    df_fuel_TWh = pd.DataFrame(columns=index, index=["Benzin", "Diesel", "Zusammen"])

    df_conversions = pd.DataFrame(
        columns=["Faktor", "Einheit"],
        index=["Benzin", "Diesel"],
        data=[[8.777777778, "kWh/L"], [9.95, "kWh/L"]],
    )
    ...
    for v, idx in zip(p, index):
        ...
        df_fuel_TWh.iloc[2, :] = df_fuel_TWh.iloc[0, :] + df_fuel_TWh.iloc[1, :]
        # \
    ...
    ws.range(last_column).end("up").offset(0, 1).value = "Treibstoffverbrauch"
