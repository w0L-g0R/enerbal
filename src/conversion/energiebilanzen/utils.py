...
def create_indices(provinces: List, last_year: int):
    ...
    res_conversion_factors = np.around(res_row_midx.get_level_values("IDX_4"), 8)
...
def preprocess_res_sheet(
    ...
    if deviations is not None:
        ...
        # Differently written in EB_Oe_88_18
        # if "Verbrauch Sektor Energie: E5 & E7 (TJ)" in deviations:
        #     deviations = deviations.drop(["Verbrauch Sektor Energie: E5 & E7 (TJ)"])

        # if "Verluste: E5 & E7 (TJ)" in deviations:
        #     deviations = deviations.drop(["Verluste: E5 & E7 (TJ)"])
        deviation_idx = []
        for index in [
