...
def test_thg_es_per_year(
    ...
    test_data_objects = [
        _data for _data in test_dataset.objects.filter(stacked_emittent_shares=True)
    ]

    test_write_to_xlsx(
        wb=test_thg_workbook, data_objects=test_data_objects, sheet_name="ES_PER_YEAR",
    )


@pytest.mark.dependency(depends=["test_thg_es_per_year"])
