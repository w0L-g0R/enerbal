...
from converter.energiebilanzen.convert.preprocess_renewables import (
...
def assign_sectors_consumption_table(
    ...
    if start_index == [-1]:
        sectors_df.loc[:, IDX[province, energy_source, :]] = float("NaN")
    ...
    # Break if the consumption is not in terajoules
    ...
    sector_columns = sectors_df.loc[:, IDX[province, energy_source, :]].columns

    df.columns = sector_columns

    sectors_df.loc[:, IDX[province, energy_source, :]] = df
...
def assign_sector_energy_consumption_table(
    ...
    if start_index == [-1]:
        ...
        sector_energy_df.loc[:, IDX[province, energy_source, :]] = float("NaN")

        sector_energy_idx_check_df[energy_source] = False
        return
