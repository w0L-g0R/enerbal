...
class xlsx:
    ...
    def __init__(self, name: str, path: Union[str, Path], sheets: List):
        super().__init__()
    ...
    def add(self, data: Data, ws: Worksheet, scale: str = "Absolute", shares: str = None, info: bool = False, ):
        ...
        if info:
            ...
            df = pd.DataFrame(
                index=["Title", "Data", "Scale",
                       "Source", "Created", "Chart"],
                data=[
                    data.title,
                    data.name,
                    scale,
        ...
        for col in ws.iter_cols(ws.min_column):
            for cell in col:
                if cell.value == "Title":
                    ...
                    style_info_index(ws=ws, cell=cell, width=self.width_df,
                                     height=self.height_info)
